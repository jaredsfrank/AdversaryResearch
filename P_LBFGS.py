...
class P_LBFGS(adversaries.Adverarial_Base):
  ...
  def create_1000_batches(self, target_class, image_reg, lr):
    ...
    for i in range(1000):
      ...
      successes += success
      np.savetxt("/scratch/jsf239/{}succ_percent.csv".format(original_subdir), np.array([successes/float(i*self.batch_size)]), delimiter = ",")
  ...
  def adversary_batch(self, data, model, target_class, image_reg, lr):
    ...
    opt = optim.Adam(self.generator_hack(inputs), lr=lr, momentum=0.9)
