...
############################################
class DeepFool(adversaries.Adverarial_Base):
    ...
    while self.check_iters(iters) and not self.all_changed(original_labels, predicted_classes):
      ...
      if self.check_iters(iters) and self.all_changed(original_labels, predicted_classes):
        if self.show_images:
          ...
          true_loss = self.CE_MSE_loss(inputs, outputs, old_images, original_labels, image_reg)
          true_loss.backward()
          true_outputs = model(inputs)
          w = inputs.grad
          trick_loss = self.CE_MSE_loss(inputs, outputs, old_images, new_labels, image_reg)
          trick_loss.backward()
          w -= inputs.grad
          new_labels = self.target_class_tensor(target_class, outputs, original_labels)
